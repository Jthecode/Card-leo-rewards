...
/**
 * IMPORTANT:
 * Once Access Development gives the exact enrollment endpoint,
 * add it to Vercel as:
 *
 * ACCESS_ENROLL_ENDPOINT=/exact/path/from/access/docs
 *
 * Example only:
 * ACCESS_ENROLL_ENDPOINT=/api/members/enroll
 */
const ACCESS_ENROLL_ENDPOINT =
  process.env.ACCESS_ENROLL_ENDPOINT || "/api/members/enroll";
...
function buildAccessEnrollmentPayload(member) {
  const fullName = getFullName(member);
  const firstName = normalizeString(member.first_name);
  const lastName = normalizeString(member.last_name);
  const email = normalizeEmail(member.email);
  const phone = normalizeString(member.phone);
  const externalMemberId = makeExternalMemberId(member);
...
function getAccessMemberId(accessResult, member) {
  const accessMemberId = readNestedValue(accessResult, [
    "memberId",
    "member_id",
    "accessMemberId",
    "access_member_id",
  ...
  return accessMemberId || makeExternalMemberId(member);
}

function getAccessSsoUrl(accessResult) {
...
async function callAccessEnrollment(member) {
  ...
  const url = buildAccessApiUrl(ACCESS_ENROLL_ENDPOINT);
  const payload = buildAccessEnrollmentPayload(member);
  ...
  try {
    ...
    return {
      ...
      message:
        error?.message ||
        "Unable to connect to Access Development enrollment API.",
      result: null,
      url,
    };
  }
  ...
  if (!response.ok) {
    return {
      ...
      message:
        ...
        `Access Development enrollment failed with status ${response.status}.`,
  ...
  return {
    ok: true,
    status: response.status,
    message: "Access Development enrollment successful.",
    result,
    url,
  };
}

async function updateSignupAccessFields({
  memberId,
  accessMemberId,
  accessStatus,
  accessSsoUrl,
  ...
  const updatePayload = {
    access_member_id: accessMemberId || null,
    access_enrollment_status: accessStatus || "enrolled",
    access_enrolled_at: new Date().toISOString(),
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error) {
      console.error("Access enrollment member lookup error:", error);
    ...
    if (member.access_member_id) {
      return sendJson(res, 200, {
        ...
        access_enrollment_status:
          member.access_enrollment_status || "enrolled",
        ...
        message: "Member is already enrolled in Access Perks.",
      });
    }

    const accessEnrollment = await callAccessEnrollment(member);

    if (!accessEnrollment.ok) {
      console.error("Access enrollment failed:", accessEnrollment);

      return sendJson(res, 502, {
        success: false,
        ok: false,
        message: accessEnrollment.message,
        access_status: accessEnrollment.status,
        access_endpoint: ACCESS_ENROLL_ENDPOINT,
        access_url: accessEnrollment.url,
        access_result: accessEnrollment.result,
      });
    }

    const accessMemberId = getAccessMemberId(accessEnrollment.result, member);
    const accessSsoUrl = getAccessSsoUrl(accessEnrollment.result);

    const updateResult = await updateSignupAccessFields({
      memberId: member.id,
      accessMemberId,
    ...
    if (updateResult.error) {
      console.error("Access enrollment Supabase update failed:", updateResult.error);

      return sendJson(res, 500, {
        success: false,
        ok: false,
        message:
          "Access enrollment worked, but we could not save it to the member account.",
      });
    }
    ...
    return sendJson(res, 200, {
      ...
      access_enrollment_status: "enrolled",
      access_sso_url: accessSsoUrl,
      access_perks_url: ACCESS_PERKS_URL,
      customer_service_phone: ACCESS_CUSTOMER_SERVICE_PHONE,
      message: "Access Perks enrollment completed.",
    });
  } catch (error) {
    console.error("Card Leo Access enrollment error:", error);
    ...
    return sendJson(res, 500, {
      ...
      message:
        error?.message ||
        "Something went wrong while enrolling the member in Access Perks.",
    });
  }
}