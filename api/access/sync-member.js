...
async function saveAccessSuccess(member, accessResult) {
...
async function saveAccessFailure(member, error) {
  const now = new Date().toISOString();

  const payload = {
    access_member_identifier: buildMemberCustomerIdentifier(member),
    access_member_status: "sync_failed",
    access_synced_at: null,
    access_sync_error:
      error?.message || "Access AMT sync failed for this member.",
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!isAccessActiveMember(member)) {
      return sendJson(res, 400, {
        success: false,
        ok: false,
        message:
          "Member is not active/paid yet. Only active paid members should be synced to Access AMT.",
        member: getSafeMemberForResponse(member),
        config: debug ? getAccessAmtConfigForDebug() : undefined,
      });
    }
    ...
    if (dryRun) {
      return sendJson(res, 200, {
        ...
        ok: true,
        dry_run: true,
        message: "Dry run only. Member was not sent to Access AMT.",
        member: getSafeMemberForResponse(memberForAccess),
        access_member_identifier: accessMemberIdentifier,
        config: debug ? getAccessAmtConfigForDebug() : undefined,
      });
    }
    ...
    try {
      ...
      const updatedMember = await saveAccessSuccess(member, accessResult);

      return sendJson(res, 200, {
        success: true,
        ok: true,
        message: "Member synced to Access AMT successfully.",
      ...
      const updatedMember = await saveAccessFailure(member, accessError);
    ...
    console.error("Card Leo access/sync-member error:", error);

    return sendJson(res, 500, {
      success: false,
      ok: false,
      message:
        error?.message || "Unable to sync member to Access AMT right now.",
    });
  }
}