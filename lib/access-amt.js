...
function getAccessAmtEndpointPath() {
  ...
  // Austin still needs to confirm the exact AMT endpoint path.
  // Once he confirms, set ACCESS_AMT_ENDPOINT_PATH in Vercel.
  // Common possibilities may be /import or /members, but do not guess in production.
  return "/import";
}
...
function getAuthHeaders(token) {
  ...
  return {
    ...
    // Access told you the “Current Access Token” is used for Offers API and Member AMT API.
    // If Access requires a different header name, change it here only.
...
function getAccessAmtConfigForDebug() {
  return {
    ...
    endpointPath: getAccessAmtEndpointPath(),
    url: getAccessAmtUrl(),
