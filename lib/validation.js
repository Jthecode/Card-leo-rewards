...
function pickFirst(payload = {}, keys = [], fallback = "") {
...
export function validateResetPasswordInput(payload = {}) {
  const errors = {};
  const token = clean(
    pickFirst(payload, ["token", "access_token", "code", "recovery_token"])
  );
  const password = String(payload.password || "");
...
export function validateProfileInput(payload = {}) {
  ...
  const firstName = normalizeName(pickFirst(payload, ["firstName", "first_name"]));
  const lastName = normalizeName(pickFirst(payload, ["lastName", "last_name"]));
...
export function validatePortalSettingsInput(payload = {}) {
  ...
  const marketingEmailOptIn = normalizeBoolean(
    payload.marketingEmailOptIn ?? payload.notifPartners
  );
  const smsOptIn = normalizeBoolean(payload.smsOptIn ?? payload.notifSms);
  const rewardReminders = normalizeBoolean(
    payload.rewardReminders ?? payload.notifRewards
  );
  const securityAlerts = normalizeBoolean(
    payload.securityAlerts ?? payload.securityLoginAlerts
  );
  const supportNotifications = normalizeBoolean(
    payload.supportNotifications ?? payload.notifAlerts
  );
...
export function validateSupportInput(payload = {}) {
  ...
  const fullName = normalizeName(
    pickFirst(payload, ["fullName", "full_name", "name"])
  );
  const email = normalizeEmail(payload.email);
...
export function validateContactInput(payload = {}) {
  ...
  const name = normalizeName(
    pickFirst(payload, ["name", "fullName", "full_name"])
  );
  const email = normalizeEmail(payload.email);
  const phone = normalizePhone(payload.phone);
  const topic = clean(payload.topic || payload.inquiryType || "general");
  const message = clean(payload.message);
...
export function validateSignupInput(payload = {}) {
  ...
  const firstName = normalizeName(
    pickFirst(payload, ["firstName", "first_name"])
  );
  const lastName = normalizeName(
    pickFirst(payload, ["lastName", "last_name"])
  );
  const email = normalizeEmail(payload.email);
  const phone = normalizePhone(payload.phone);
  const city = clean(payload.city);
  const state = clean(payload.state);
  const referralName = clean(
    pickFirst(payload, [
      "referralName",
      "referral_name",
      "sponsorName",
  ...
  const agreed = normalizeBoolean(payload.agreed ?? payload.agree);
